...
export default function App() {
  ...
  const updateCartQty = (plantId, qty) => {
    ...
    const maxQ = plant.quantity;
    const clampedQty = Math.min(qty, maxQ);
    setCart(prev => {
      const existing = prev.find(c => c.plantId === plantId);
      if (existing) {
        return prev.map(c => c.plantId === plantId ? { ...c, quantity: clampedQty } : c);
      }
  ...
  if (!loaded) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", fontFamily: "'DM Sans', sans-serif", background: "#f7f5f0" }}>
      <div style={{ textAlign: "center", color: "#6b7c5e" }}>
        <Icon name="leaf" size={40} />
        <div style={{ marginTop: 12, fontSize: 14, letterSpacing: 2 }}>LOADING INVENTORY...</div>
      </div>
    </div>
  );
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", minHeight: "100vh", background: "#f7f5f0", color: "#2c2c2c" }}>
      ...
      {toast && (
        <div style={{
          ...
          background: toast.type === "error" ? "#c0392b" : "#4a6741",
          color: "#fff", padding: "12px 20px", borderRadius: 8,
          fontSize: 14, fontWeight: 500, boxShadow: "0 4px 20px rgba(0,0,0,.15)",
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .order-input:not(:placeholder-shown) { background: #edf7ea; font-weight: 700; }
      `}</style>

      {view === "shop" ? (
        <ShopView plants={plants} cart={cart} updateCartQty={updateCartQty}
          removeFromCart={removeFromCart} submitOrder={submitOrder} showToast={showToast}
          onGoAdmin={() => setView("admin")} />
      ) : (
        <AdminView plants={plants} orders={orders} savePlants={savePlants} saveOrders={saveOrders}
          showToast={showToast} onGoShop={() => setView("shop")} />
      )}
    </div>
  );
}
...
function ShopView({ plants, cart, updateCartQty, removeFromCart, submitOrder, showToast, onGoAdmin }) {
  ...
  const handleCheckout = (info) => {
    const order = submitOrder(info);
    setCheckoutOpen(false);
    setCartOpen(false);
    setOrderConfirm(order);
  };

  const getCartQty = (plantId) => {
  ...
  const columns = [
    ...
    { key: "variety", label: "Color / Variety", width: "14%" },
    { key: "price", label: "Price", width: "8%", align: "right" },
    { key: "quantity", label: "Available", width: "9%", align: "center" },
    { key: "packSize", label: "Pack Size", width: "8%", align: "center" },
    { key: "comments", label: "Comments", width: "18%" },
    { key: "order", label: "Order Qty", width: "8%", align: "center", noSort: true },
  ];

  const thBase = {
    padding: "10px 12px", fontSize: 11, fontWeight: 700, letterSpacing: 1.2,
    textTransform: "uppercase", color: "#fff", background: "#4a6741",
    position: "sticky", top: 0, zIndex: 2, cursor: "pointer",
    userSelect: "none", whiteSpace: "nowrap", borderRight: "1px solid rgba(255,255,255,.15)",
  };
  const tdBase = {
    padding: "8px 12px", fontSize: 13, borderBottom: "1px solid #e8e4dc",
    borderRight: "1px solid #f0ede6", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
  };
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "12px 24px 30px" }}>
        <div style={{ background: "#fff", borderRadius: 10, border: "1px solid #ddd", overflow: "auto", boxShadow: "0 1px 8px rgba(0,0,0,.05)" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900 }}>
            ...
            <tbody>
              {filtered.map((plant, i) => {
                ...
                return (
                  <tr key={plant.id} className="row-hover"
                    style={{
                      background: isInCart ? "#f8fdf6" : i % 2 === 0 ? "#fff" : "#fafaf7",
                    ...
                    <td style={{ ...tdBase, fontWeight: 600 }}>{plant.name}</td>
                    <td style={{ ...tdBase, color: "#777", fontStyle: "italic" }}>{plant.variety || "—"}</td>
                    <td style={{ ...tdBase, textAlign: "right", fontWeight: 600, fontFamily: "'DM Sans', sans-serif" }}>{fmt(plant.price)}</td>
                    <td style={{
                      ...tdBase, textAlign: "center", fontWeight: 700,
                      color: outOfStock ? "#c0392b" : lowStock ? "#e67e22" : "#333",
                    }}>
                      {outOfStock ? <span style={{ fontSize: 11, fontWeight: 600 }}>SOLD OUT</span> : plant.quantity}
                    </td>
                    <td style={{ ...tdBase, textAlign: "center", color: "#888" }}>{plant.packSize || 1}</td>
                    <td style={{ ...tdBase, color: "#888", fontSize: 12, maxWidth: 200 }}>{plant.comments || "—"}</td>
                    <td style={{ ...tdBase, textAlign: "center", borderRight: "none", padding: "5px 8px" }}>
                      {!outOfStock ? (
                        <input
                          ...
                          style={{
                            width: 56, padding: "5px 4px", border: "1px solid #ddd", borderRadius: 5,
                            fontSize: 14, textAlign: "center", background: isInCart ? "#edf7ea" : "#fafaf8",
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 12, opacity: .7 }}>
                {cart.map(c => `${c.plantName} ×${c.quantity}`).join("  ·  ")}
              </span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      {/* Cart Drawer */}
      {cartOpen && <CartDrawer cart={cart} plants={plants} cartTotal={cartTotal} updateCartQty={updateCartQty}
        removeFromCart={removeFromCart} onClose={() => setCartOpen(false)}
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "12px 24px" }}>
          {cart.length === 0 ? (
            <div style={{ textAlign: "center", padding: 40, color: "#999" }}>
              <Icon name="cart" size={40} /><p style={{ marginTop: 12 }}>Your cart is empty</p>
            </div>
          ) : (
                    ...
                    <tr key={item.plantId} style={{ borderBottom: "1px solid #f0f0f0" }}>
                      ...
                      <td style={{ textAlign: "center" }}>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                          <button onClick={() => updateCartQty(item.plantId, item.quantity - 1)}
                            style={{ width: 24, height: 24, borderRadius: 4, background: "#f0f0f0", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <Icon name="minus" size={12} /></button>
                          <span style={{ fontWeight: 700, fontSize: 14, minWidth: 20, textAlign: "center" }}>{item.quantity}</span>
                          <button onClick={() => updateCartQty(item.plantId, Math.min(item.quantity + 1, maxQ))}
                            style={{ width: 24, height: 24, borderRadius: 4, background: "#f0f0f0", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        {cart.length > 0 && (
          <div style={{ padding: "14px 24px", borderTop: "1px solid #eee" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14, fontSize: 18, fontWeight: 700 }}>
              <span>Total</span><span style={{ color: "#4a6741" }}>{fmt(cartTotal)}</span>
            </div>
            <button onClick={onCheckout}
              style={{ width: "100%", padding: "12px", background: "#4a6741", color: "#fff", borderRadius: 10, fontSize: 15, fontWeight: 600 }}>
              Proceed to Checkout
            </button>
          </div>
        )}
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ position: "relative", background: "#fff", borderRadius: 16, maxWidth: 480, width: "100%", maxHeight: "90vh", overflow: "auto", padding: 32, animation: "fadeUp .3s ease" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div><label style={{ fontSize: 12, fontWeight: 600, color: "#666", marginBottom: 4, display: "block" }}>Name *</label>
            <input value={form.customerName} onChange={e => setForm({ ...form, customerName: e.target.value })} style={inp} placeholder="Your name or business" /></div>
          <div><label style={{ fontSize: 12, fontWeight: 600, color: "#666", marginBottom: 4, display: "block" }}>Email *</label>
            <input type="email" value={form.customerEmail} onChange={e => setForm({ ...form, customerEmail: e.target.value })} style={inp} placeholder="email@example.com" /></div>
          <div><label style={{ fontSize: 12, fontWeight: 600, color: "#666", marginBottom: 4, display: "block" }}>Phone *</label>
            <input value={form.customerPhone} onChange={e => setForm({ ...form, customerPhone: e.target.value })} style={inp} placeholder="(555) 555-0100" /></div>
          <div><label style={{ fontSize: 12, fontWeight: 600, color: "#666", marginBottom: 4, display: "block" }}>Notes (optional)</label>
            <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} style={{ ...inp, height: 70, resize: "vertical" }} placeholder="Pickup time, special requests..." /></div>
          <button onClick={handle} disabled={submitting || !form.customerName || !form.customerEmail || !form.customerPhone}
            style={{
              ...
              background: (!form.customerName || !form.customerEmail || !form.customerPhone) ? "#ccc" : "#4a6741",
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ position: "relative", background: "#fff", borderRadius: 16, maxWidth: 440, width: "100%", padding: 32, textAlign: "center", animation: "fadeUp .3s ease" }}>
        <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#e8f5e3", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", color: "#4a6741" }}>
      ...
      <header style={{ background: "#2c2c2c", color: "#fff", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 600 }}>Admin Dashboard</span>
          </div>
          <button onClick={onGoShop} style={{ color: "#fff", opacity: .7, fontSize: 12, letterSpacing: 1, textTransform: "uppercase", display: "flex", alignItems: "center", gap: 6 }}>
            <Icon name="arrowLeft" size={16} /> Back to Shop</button>
        </div>
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 24px", display: "flex", gap: 0 }}>
          {[
            ...
            { key: "upload", label: "Upload / Download", icon: "upload" },
          ].map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              ...
              {t.key === "orders" && orders.filter(o => o.status === "pending").length > 0 && (
                <span style={{ background: "#e67e22", color: "#fff", fontSize: 10, fontWeight: 700, padding: "1px 6px", borderRadius: 10, marginLeft: 4 }}>
...
function InventoryTab({ plants, savePlants, showToast }) {
  ...
  const saveEdit = () => {
    savePlants(plants.map(p => p.id === editing ? { ...editForm, price: Number(editForm.price), quantity: Number(editForm.quantity), packSize: Number(editForm.packSize) || 1 } : p));
    setEditing(null); showToast("Plant updated");
  };
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10, marginBottom: 16 }}>
        {[
          { label: "Total Plants", value: plants.length, color: "#4a6741" },
          { label: "In Stock", value: plants.filter(p => p.quantity > 0).length, color: "#27ae60" },
          { label: "Low Stock", value: lowStock.length, color: "#e67e22" },
          { label: "Out of Stock", value: outOfStock.length, color: "#c0392b" },
        ].map(s => (
      ...
      {lowStock.length > 0 && (
        <div style={{ background: "#fef9e7", border: "1px solid #f4d03f", borderRadius: 8, padding: "10px 14px", marginBottom: 12, fontSize: 12 }}>
          <b style={{ color: "#e67e22" }}>Low stock:</b> {lowStock.map(p => `${p.name} ${p.variety ? '('+p.variety+')' : ''} — ${p.quantity}`).join(", ")}
        </div>
      )}
      <div style={{ marginBottom: 10, position: "relative", width: 240 }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search inventory..."
          style={{ width: "100%", padding: "7px 7px 7px 30px", border: "1px solid #ddd", borderRadius: 6, fontSize: 12, background: "#fff" }} />
      </div>
          ...
          <tbody>
            {filtered.map(plant => editing === plant.id ? (
              <tr key={plant.id} style={{ background: "#fffff5" }}>
                ...
                <td style={cellStyle}>
                  <select value={editForm.category} onChange={e => setEditForm({ ...editForm, category: e.target.value })} style={editInp}>
                ...
                <td style={cellStyle}>
                  <div style={{ display: "flex", gap: 4 }}>
                    <button onClick={saveEdit} style={{ color: "#27ae60" }}><Icon name="check" size={16} /></button>
                    <button onClick={cancelEdit} style={{ color: "#999" }}><Icon name="x" size={16} /></button>
                  </div></td>
              </tr>
            ) : (
              <tr key={plant.id}>
                ...
                <td style={cellStyle}>
                  <div style={{ display: "flex", gap: 4 }}>
                    <button onClick={() => startEdit(plant)} style={{ color: "#4a6741" }}><Icon name="edit" size={14} /></button>
                    <button onClick={() => deletePlant(plant.id)} style={{ color: "#c0392b" }}><Icon name="trash" size={14} /></button>
                  </div></td>
              </tr>
            ))}
          </tbody>
...
function OrdersTab({ orders, saveOrders, plants, savePlants, showToast }) {
  ...
  const updateStatus = (orderId, newStatus) => {
    ...
    if (newStatus === "cancelled" && order && order.status !== "cancelled") {
      savePlants(plants.map(p => { const item = order.items.find(i => i.plantId === p.id); return item ? { ...p, quantity: p.quantity + item.quantity } : p; }));
    }
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
        {["all", "pending", "confirmed", "fulfilled", "cancelled"].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            ...
            background: filter === f ? "#2c2c2c" : "#fff", color: filter === f ? "#fff" : "#666",
            border: `1px solid ${filter === f ? "#2c2c2c" : "#ddd"}`, textTransform: "capitalize",
              ...
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #f0f0f0" }}>
                {order.items.map(item => (
                  <div key={item.plantId} style={{ display: "flex", justifyContent: "space-between", padding: "2px 0", fontSize: 13 }}>
                    <span>{item.plantName} <span style={{ color: "#999" }}>× {item.quantity}</span></span>
                    <span style={{ fontWeight: 600 }}>{fmt(item.quantity * item.unitPrice)}</span>
                  </div>
                ))}
...
function UploadTab({ plants, savePlants, showToast }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      const normalized = rows.map(row => {
        const r = {};
        Object.entries(row).forEach(([k, v]) => { r[k.trim().toLowerCase().replace(/[^a-z0-9]/g, "")] = v; });
        return {
          name: r.name || r.plantname || r.plant || "",
          variety: r.variety || r.color || r.colorvariety || r.cultivar || "",
          category: r.category || r.type || r.cat || "Other",
          size: r.size || r.potsize || r.containersize || "",
          price: parseFloat(r.price || r.unitprice || r.cost || 0),
          quantity: parseInt(r.quantity || r.qty || r.stock || r.available || 0, 10),
          packSize: parseInt(r.packsize || r.pack || r.traysize || 1, 10) || 1,
          comments: r.comments || r.notes || r.description || r.comment || "",
        };
      }).filter(r => r.name);
      setPreview(normalized);
      showToast(`Parsed ${normalized.length} plants from ${file.name}`);
  ...
  const applyUpload = () => {
    ...
    if (mode === "replace") { updated = preview.map(p => ({ ...p, id: genId() })); }
      ...
      preview.forEach(incoming => {
        const idx = updated.findIndex(p => p.name.toLowerCase() === incoming.name.toLowerCase() && (p.variety || "").toLowerCase() === (incoming.variety || "").toLowerCase());
  ...
  const downloadCSV = () => {
    const csv = Papa.unparse(plants.map(p => ({ Name: p.name, "Color / Variety": p.variety || "", Category: p.category, Size: p.size, Price: p.price, Quantity: p.quantity, "Pack Size": p.packSize, Comments: p.comments })));
    const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([csv], { type: "text/csv" }));
    a.download = `plant-inventory-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
  };
  const downloadXLSX = () => {
    const ws = XLSX.utils.json_to_sheet(plants.map(p => ({ Name: p.name, "Color / Variety": p.variety || "", Category: p.category, Size: p.size, Price: p.price, Quantity: p.quantity, "Pack Size": p.packSize, Comments: p.comments })));
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Inventory");
    XLSX.writeFile(wb, `plant-inventory-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
    ...
    <div>
      <div style={{ background: "#fff", borderRadius: 10, border: "1px solid #e8e4dc", padding: 20, marginBottom: 16 }}>
        ...
        <p style={{ color: "#888", fontSize: 12, marginBottom: 14 }}>
          Upload a CSV or XLSX. Expected columns: Name, Color/Variety, Category, Size, Price, Quantity, Pack Size, Comments. Column names are matched flexibly.
        </p>
            ...
            <div style={{ maxHeight: 250, overflow: "auto", borderRadius: 6, border: "1px solid #e8e4dc" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 500 }}>
                ...
                <tbody>{preview.slice(0, 50).map((p, i) => (
                  <tr key={i}>
                    <td style={{ ...cellStyle, fontWeight: 600 }}>{p.name}</td><td style={{ ...cellStyle, fontStyle: "italic" }}>{p.variety}</td>
                    <td style={cellStyle}>{p.category}</td><td style={cellStyle}>{p.size}</td>
                    <td style={{ ...cellStyle, textAlign: "right" }}>{fmt(p.price)}</td><td style={{ ...cellStyle, textAlign: "right" }}>{p.quantity}</td>
                    <td style={{ ...cellStyle, textAlign: "right" }}>{p.packSize}</td><td style={{ ...cellStyle, color: "#888" }}>{p.comments}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
